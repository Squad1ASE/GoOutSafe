...
class User(db.Model):
    ...
    # __table_args__ = (CheckConstraint(len ( password ) >= 8, name='check_length_password'), {})
...
class Table(db.Model):
    ...
    restaurant = relationship('Restaurant', foreign_keys='Table.restaurant_id')  # TO CHECK if it is really used
...
class WorkingDay(db.Model):
    ...
    restaurant_id = db.Column(db.Integer, db.ForeignKey('restaurant.id'))
    restaurant = relationship('Restaurant', foreign_keys='WorkingDay.restaurant_id')  # TO CHECK if it is really used

    work_shifts = db.Column(db.Enum)  # or is better to serialize a python object?
    day = db.Column(db.Integer, nullable=False)  # 0=Mon, 1=Tue, ...
